...
Name : 3. MVM-Senior Citizens Living Alone
Group : 
With QGIS : 32815
"""
...
class MvmseniorCitizensLivingAlone(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        living_alone_style = os.path.join(qml_base_path, '6. SC Living Alone.qml')
        ...
        # Load layer into project (SC)
        alg_params = {
            'INPUT': outputs['CreatePointsLayerFromTable']['OUTPUT'],
            'NAME': 'CityMun_SC Living Alone'
        }
        outputs['LoadLayerIntoProjectSc'] = processing.run('native:loadlayer', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        # SC Living Alone
        alg_params = {
            'INPUT': outputs['LoadLayerIntoProjectSc']['OUTPUT'],
            'STYLE': living_alone_style
        }
        outputs['ScLivingAlone'] = processing.run('native:setlayerstyle', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        return results

    def name(self):
        return '3. MVM-Senior Citizens Living Alone'
    ...
    def createInstance(self):
        return MvmseniorCitizensLivingAlone()
